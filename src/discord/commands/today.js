const { getPlayerDataToday } = require("../../api/getPlayerDataToday");
...
async function handlePlayerTodayCommand(message, args) {
  ...
  try {
    ...
    const playerData = await getPlayerDataToday(playerName);
    ...
    const pvpScoreboard = generatePvPScoreboard(playerName, history) || [];
    const formattedInfo = formatPlayerInfo(playerData, playerName, pvpScoreboard);
